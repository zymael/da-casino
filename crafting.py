...
(db.craft_item, atomic) plus the resulting item grant -- always stored, never auto-equipped, for
equipment (same as ordinary loot -- see db.store_equipment_item; the player equips manually via
!equipment), or added to the generic `inventory` table for every other output_kind (consumable,
quest_item, horse_clothes, housing_item -- see _INVENTORY_REGISTRIES). Mirrors quests.py's turn_in
and shop.py's buy -- composes several separately-atomic db calls rather than one all-encompassing
transaction, since the consumption step and the grant step touch different tables (inventory vs.
character_equipment/equipment_inventory) that can't safely share one open connection/transaction.
"""
...
async def craft(guild_id: int, user_id: int, recipe_id: str) -> dict:
    ...
    "output_item", "output_kind"}. On failure (status "insufficient_materials" or "broke"),
    success is False and output_item is None -- nothing was consumed or granted."""
    ...
    if status != "ok":
        return {
            ...
            "output_item": None, "output_kind": None,
        }
    ...
    return {
        ...
        "output_item": item, "output_kind": recipe["output_kind"],
    }
...
async def combine(guild_id: int, user_id: int, material_ids: list[str]) -> dict:
    ...
    if recipe is None:
        return {
            ...
            "output_item": None, "output_kind": None, "newly_discovered": False,
        }
