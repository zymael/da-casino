...
import room_view
import rooms
from roulette_view import (
    DAUGHTERS_NEGLECT_FLAG,
    DAUGHTERS_TARGET_ID,
    TAXMAN_LEGS_BROKEN_FLAG,
    TAXMAN_TARGET_ID,
    active_rounds,
    start_roulette_table,
)
import slots_view
...
bot = commands.Bot(
    command_prefix="!", intents=intents, help_command=None, case_insensitive=True, strip_after_prefix=True,
)
...
async def pizza(ctx):
    ...
    # Ordering pizza is what makes you wonder where it's coming from -- pizza_devotee is the
    # unlock_trigger on the Big Cheeze Pizza Palace delve (dungeon_delves.json), so this is the one
    # place that gate opens. Awarded on any successful order rather than at a count: try_award_many
    # is idempotent per user, so the second pizza onward is a no-op with no bookkeeping needed here.
    await achievements.try_award_many(
